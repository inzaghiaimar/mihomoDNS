...
- [安装参数详解](#安装参数详解)
- [机场个性化配置](#机场个性化配置)
- [⚠️ 必读：示例 IP 地址说明（新手请先看）](#️-必读示例-ip-地址说明新手请先看)
...
- [PVE / ESXi 部署（精简版）](#pve--esxi-部署精简版)
  - [选型：LXC vs KVM vs ESXi VM](#选型lxc-vs-kvm-vs-esxi-vm)
  - [PVE LXC 容器（推荐，纯 DNS+SOCKS 模式）](#pve-lxc-容器推荐纯-dnssocks-模式)
  - [PVE KVM 虚拟机](#pve-kvm-虚拟机)
  - [ESXi 虚拟机](#esxi-虚拟机)
  - [部署后验证（LXC/KVM/ESXi 通用）](#部署后验证lxckvmesxi-通用)
  - [虚拟化常见坑](#虚拟化常见坑)
- [安装后使用](#安装后使用)
- [卸载](#卸载)
- [目录结构](#目录结构)
...
### ⚠️ 必读：示例 IP 地址说明（新手请先看）

本文档所有命令中的 IP 地址均为**示例**，请**严格按你实际的局域网网段替换**，直接粘贴会导致网络不通。

> 查看你自己的实际 IP：
> ```bash
> # Linux / mihomoDNS 服务器上
> ip addr show | grep 'inet '
> ip route show | grep default    # 查看默认网关 = 主路由 IP
>
> # ROS 上
> /ip address print
...
| 文档中出现的示例 IP | 含义 | 你应该替换成 |
| --- | --- | --- |
| `192.168.1.1` | **主路由** LAN IP（ROS / 爱快的网关地址） | 你的主路由 LAN IP，例如 `192.168.5.1` / `192.168.88.1` / `10.0.0.1` |
| `192.168.1.2` | **mihomoDNS 服务器**（旁路由/DNS 服务器）的固定 IP | 你部署 mihomoDNS 的 LXC 容器 / 虚拟机 / 物理机的 LAN IP |
| `192.168.1.10`、`192.168.1.x` | 局域网客户端（PC / 手机等） | 保留占位，或替换成你局域网对应客户端的 IP |
| `192.168.1.0/24` | 整个局域网网段（CIDR） | 你实际的网段 CIDR，例 `192.168.5.0/24` / `192.168.88.0/24` |
| `vmbr0`（PVE）/ `VM Network`（ESXi） | 桥接物理网卡的**虚拟交换机/端口组** | 你实际连接物理 LAN 的虚拟交换机名称（`pveam` 创建时默认 `vmbr0`，ESXi 通常为 `VM Network`） |
| CTID=100 / VMID=101 | PVE 容器/虚拟机 ID（数字 ID） | 若有冲突，改成你 PVE/ESXi 上未占用的 ID |
| 端口 `7890`、`9090`、`1053`、`5335`、`9099` | clash + mosdns 默认端口（见端口表） | 除非你改了 `.env`，否则**不要动** |

**替换原则：**
1. 先执行命令查出**自己的**主路由 IP（默认网关）和 mihomoDNS 服务器 IP，写下来，**再**开始复制命令。
2. 命令里凡是 `192.168.1.1` 一律换成你的主路由 IP，`192.168.1.2` 一律换成你的 mihomoDNS IP，`192.168.1.0/24` 换成你的网段 CIDR。
3. 静态 IP、网关、DNS 三个必须**在同一网段**（例：主路由 `192.168.5.1` → mihomoDNS 服务器必须是 `192.168.5.x/24`，不能写成 `192.168.1.2`），否则容器/虚拟机直接上不了网。
...
> ⚠️ **复制命令前请先替换 IP**
>
> 本章节所有命令中的 IP 都是**示例**，请先执行以下命令查出**你自己的实际 IP**，再批量替换：
> ```bash
> # ① mihomoDNS 服务器上 — 查出本机 IP + 主路由 IP（默认网关）
> ip route show | grep default
> # 输出示例：default via 192.168.5.1 dev eth0 src 192.168.5.6
> #                                ↑ 主路由IP           ↑ mihomoDNS IP
>
> # ② ROS 上 — 查出 LAN 网段
...
> ```
>
> 下文示例对照：主路由 `192.168.1.1` → 换成你的主路由 IP，mihomoDNS `192.168.1.2` → 换成你的 mihomoDNS IP。
...
> ⚠️ **创建容器/虚拟机前请先确认你的网段**
>
> 先在 PVE / ESXi 宿主机或现有设备上执行：
> ```bash
> ip route show | grep default
> # 记下：主路由 IP（via 后面的）、当前网段（dev 后面网卡所属的子网）
> ```
>
> 本章节以 `192.168.1.1`（主路由）、`192.168.1.2`（mihomoDNS）、`vmbr0`（PVE 默认桥接网卡）为**示例**，请严格替换成你自己的实际值。同一网段原则：主路由、mihomoDNS、客户端三者必须在同一个子网（例：都在 `192.168.5.x`），否则直接部署后会 ping 不通、上不了网。
>
> 以下保留核心步骤。
